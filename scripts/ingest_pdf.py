...
async def ingest_document(
    doc_service: DocumentService, article: Dict[str, Any], metadata: Dict[str, Any]
) -> None:
...
async def ingest_pdf(pdf_path: str, is_constitution: bool = True) -> None:
    ...
    json_path = (
        ...
        if is_constitution
        else output_dir / "legal_document.json"
    )
    ...
    for article in articles:
        ...
        print(
            f"Saving article {article['number']} with length {len(article['content'])}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest a PDF document into the vector store"
    )
    parser.add_argument("pdf_path", help="Path to the PDF file")
    parser.add_argument(
        "--constitution", action="store_true", help="Process as Constitution"
    )
